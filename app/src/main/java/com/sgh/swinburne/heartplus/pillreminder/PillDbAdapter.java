...
public class PillDbAdapter extends SQLiteOpenHelper {
    ...
    private static final String KEY_ALARMS_DOSE = "dose";
    private static final String KEY_ALARMS_INSTRUCTION = "instruction";
    ...
    private static final String CREATE_ALARM_TABLE =
            "create table " + ALARM_TABLE + "("
                    ...
                    + KEY_ALARMS_DOSE + " text not null,"
                    + KEY_ALARMS_INSTRUCTION + " text not null,"
     ...
     * Constructor
     */
    public PillDbAdapter(Context context) {
...
// ############################## create methods ###################################### //
    ...
    public long createPill(Pill pill) {
        ...
        values.put(KEY_DOSE, pill.getDose());
        values.put(KEY_INSTRUCTIONS, pill.getInstruction());
    ...
    public long[] createAlarm(Alarm alarm, long pill_id) {
        ...
        for (boolean day : alarm.getDayOfWeek()) {
            if (day) {
                ...
                values.put(KEY_ALARMS_DOSE, alarm.getdose());
                values.put(KEY_ALARMS_INSTRUCTION, alarm.getinstruction());
...
// ############################# get methods ####################################### //
    ...
    public Pill getPillByName(String pillName) {
        ...
        if (c.moveToFirst() && c.getCount() >= 1) {
            ...
            pill.setDose(c.getString(c.getColumnIndex(KEY_DOSE)));
            pill.setInstruction(c.getString(c.getColumnIndex(KEY_INSTRUCTIONS)));
    ...
    public List<Pill> getAllPills() {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                p.setDose(c.getString(c.getColumnIndex(KEY_DOSE)));
                p.setInstruction(c.getString(c.getColumnIndex(KEY_INSTRUCTIONS)));
    ...
    public List<Alarm> getAllAlarmsByPill(String pillName) throws URISyntaxException {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                al.setdose(c.getString(c.getColumnIndex(KEY_ALARMS_DOSE)));
                al.setinstruction(c.getString(c.getColumnIndex(KEY_ALARMS_INSTRUCTION)));
    ...
    private List<Alarm> combineAlarms(List<Alarm> dbAlarms) throws URISyntaxException {
        ...
        for (Alarm al : dbAlarms) {
            if (timesOfDay.contains(al.getStringTime())) {
                ...
                newAlarm.setdose(al.getdose());
                newAlarm.setinstruction(al.getinstruction());
...
// ############################### delete methods##################################### //
    ...
    public void deletePill(String pillName) throws URISyntaxException {
        ...
        db.delete(PILL_TABLE, KEY_PILLNAME
                + " = ?", new String[]{pillName});
       // db.delete(PILL_TABLE, KEY_DOSE
        //        + " = ?", new String[]{dose});
      //  db.delete(PILL_TABLE, KEY_INSTRUCTIONS
